...
<body>
    ...
    <form action="info_ok.jsp" name="regform" id="regform" method="post" onsubmit="return sendit()">
        ...
        %>
        ></label> <label>카페<input type="checkbox" name="hobby" value="카페"
        <%
        for(int i=0; i<hobbyArr.length; i++){
         if(hobbyArr[i].equals("카페")){
            %>checked="checked"<%
         }
      }
        %>
        ></label> <label>잠자기<input type="checkbox" name="hobby" value="잠자기"
        <%
        for(int i=0; i<hobbyArr.length; i++){
         if(hobbyArr[i].equals("잠자기")){
            %>checked="checked"<%
         }
      }
        %>></label></p>
